...
export async function recordPublication(input: {
  ...
  draftId: string;
  versionId?: string;
  eventType: "published" | "rollback_requested" | "rollback_published" | "rollback_failed";
  snapshot: PublicationSnapshot;
  publishedUrl?: string;
  jobId?: string;
  createdBy?: string;
}) {
  const [row] = await supabaseAdmin<UnknownRecord[]>("draft_publication_history", {
    method: "POST",
    body: JSON.stringify({
...
export async function listPublicationHistory(workspaceId: string, draftId: string) {
  return supabaseAdmin<UnknownRecord[]>(
    ...
    {},
    `?workspace_id=eq.${encodeURIComponent(workspaceId)}&draft_id=eq.${encodeURIComponent(draftId)}&order=created_at.desc`
  );
}
