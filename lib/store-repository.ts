...
function isRemoteStore(ctx?: StoreContext) {
...
export async function listDraftsRemote(ctx: StoreContext) {
  if (!isRemoteStore(ctx)) return local.listDrafts();
  return supabaseRest<local.ContentDraft[]>(
    ctx.req!,
    "drafts",
    {},
...
export async function addDraftRemote(
  ctx: StoreContext,
  draft: Omit<local.ContentDraft, "id" | "status" | "createdAt">
) {
  if (!isRemoteStore(ctx)) return local.addDraft(draft);
  const row = {
    ...
    suggested_headings: draft.suggestedHeadings ?? null,
    schema_jsonld: draft.schemaJsonLd ?? null,
    status: "pending",
  };
  const [created] = await supabaseRest<UnknownRecord[]>(ctx.req!, "drafts", {
    method: "POST",
...
export async function updateDraftRemote(ctx: StoreContext, id: string, patch: Partial<local.ContentDraft>) {
  if (!isRemoteStore(ctx)) return local.updateDraft(id, patch);
  ...
  for (const [key, value] of Object.entries(patch)) {
    const map: Record<string, string> = {
      ...
      suggestedHeadings: "suggested_headings",
...
export async function getPublishSettingsRemote(ctx: StoreContext): Promise<local.PublishSettings> {
  if (!isRemoteStore(ctx)) return local.getPublishSettings();
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req!,
    "connections",
    {},
    q({ workspace_id: `eq.${ctx.workspaceId}`, status: "neq.revoked" })
  );
  ...
  for (const row of rows) {
    try {
      ...
      if (row.provider === "wordpress")
        result.website = { platformType: "wordpress", wordpress: data, permission: data.permission ?? "suggest" };
      if (row.provider === "shopify")
        result.website = { platformType: "shopify", shopify: data, permission: data.permission ?? "suggest" };
      if (row.provider === "custom")
        result.website = { platformType: "custom", custom: data, permission: data.permission ?? "suggest" };
      ...
      /* never expose corrupt credentials */
    }
  }
  return result;
}
...
export async function savePublishSettingsRemote(ctx: StoreContext, partial: Partial<local.PublishSettings>) {
  if (!isRemoteStore(ctx)) return local.savePublishSettings(partial);
  ...
  if (partial.youtube)
    entries.push(["youtube", { ...partial.youtube.settings, permission: partial.youtube.permission }]);
  if (partial.facebook)
    entries.push(["facebook", { ...partial.facebook.settings, permission: partial.facebook.permission }]);
...
export async function recordSeoScoreRemote(
  ctx: StoreContext,
  url: string,
  score: number,
  ...
  if (!isRemoteStore(ctx)) return local.recordSeoScore(url, score);
  await supabaseRest(ctx.req!, "seo_score_history", {
    method: "POST",
    body: JSON.stringify({
      workspace_id: ctx.workspaceId,
      url,
      score,
      deterministic_score: deterministicScore ?? null,
...
export async function getSeoScoreHistoryRemote(ctx: StoreContext, url?: string) {
  if (!isRemoteStore(ctx)) return local.getSeoScoreHistory(url);
  const params: Record<string, string> = { workspace_id: `eq.${ctx.workspaceId}`, order: "created_at.asc" };
  if (url) params.url = `eq.${url}`;
  const rows = await supabaseRest<UnknownRecord[]>(ctx.req!, "seo_score_history", {}, q(params));
...
export async function listCalendarItemsRemote(ctx: StoreContext) {
  if (!isRemoteStore(ctx)) return local.listCalendarItems();
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req!,
    "calendar_items",
    {},
    q({ workspace_id: `eq.${ctx.workspaceId}`, order: "scheduled_date.asc" })
  );
  return rows.map(mapCalendar);
}

export async function addCalendarItemRemote(
  ctx: StoreContext,
  item: Omit<local.CalendarItem, "id" | "status" | "createdAt">
) {
  if (!isRemoteStore(ctx)) return local.addCalendarItem(item);
  const [row] = await supabaseRest<UnknownRecord[]>(ctx.req!, "calendar_items", {
    method: "POST",
    body: JSON.stringify({
      workspace_id: ctx.workspaceId,
...
export async function updateCalendarItemRemote(ctx: StoreContext, id: string, patch: Partial<local.CalendarItem>) {
  if (!isRemoteStore(ctx)) return local.updateCalendarItem(id, patch);
  const row: Record<string, unknown> = {};
...
export async function getDueCalendarItemsRemote(ctx: StoreContext) {
  if (!isRemoteStore(ctx)) return local.getDueCalendarItems();
...
function mapDraft(r: UnknownRecord): local.ContentDraft {
  return {
    id: r.id,
    channel: r.channel,
    kind: r.kind,
    ...
    suggestedHeadings: r.suggested_headings ?? undefined,
    schemaJsonLd: r.schema_jsonld ?? undefined,
    status: r.status,
...
function mapCalendar(r: UnknownRecord): local.CalendarItem {
  return {
    id: r.id,
    channel: r.channel,
    topic: r.topic,
    ...
    status: r.status,
    createdAt: r.created_at,
    resultDraftId: r.result_draft_id ?? undefined,
    errorMessage: r.error_message ?? undefined,
  };
}

export async function createDraftVersionRemote(
  ctx: StoreContext,
  input: {
    draftId: string;
  ...
  if (!isRemoteStore(ctx)) throw new Error("Optimization versioning requires a Supabase workspace in production mode.");
  const existing = await supabaseRest<UnknownRecord[]>(
    ...
    q({
      workspace_id: `eq.${ctx.workspaceId}`,
      draft_id: `eq.${input.draftId}`,
      order: "version_number.desc",
      limit: "1",
    })
  );
  const versionNumber = (existing[0]?.version_number ?? 0) + 1;
  const [row] = await supabaseRest<UnknownRecord[]>(ctx.req!, "draft_versions", {
    method: "POST",
    body: JSON.stringify({
      workspace_id: ctx.workspaceId,
      draft_id: input.draftId,
      version_number: versionNumber,
      source: input.source ?? "ai_optimization",
      status: "proposed",
      original_content: input.original,
      optimized_content: input.optimized,
      change_decisions: input.decisions,
      warnings: input.warnings ?? [],
      created_by: input.createdBy ?? null,
      rollback_of_version_id: input.rollbackOfVersionId ?? null,
    }),
    headers: { Prefer: "return=representation" },
  });
...
export async function listDraftVersionsRemote(ctx: StoreContext, draftId: string) {
  ...
  const rows = await supabaseRest<UnknownRecord[]>(
    ...
    "draft_versions",
    {},
    q({ workspace_id: `eq.${ctx.workspaceId}`, draft_id: `eq.${draftId}`, order: "version_number.desc" })
  );
...
function mapDraftVersion(r: UnknownRecord) {
  return {
    id: r.id,
    draftId: r.draft_id,
    ...
    optimized: r.optimized_content,
    decisions: r.change_decisions ?? [],
    warnings: r.warnings ?? [],
    createdBy: r.created_by ?? undefined,
    createdAt: r.created_at,
    rollbackOfVersionId: r.rollback_of_version_id ?? undefined,
