...
export interface ContentStrategyProject {
  id: string;
  workspaceId: string;
  name: string;
...
const q = (p: Record<string, string>) =>
  ...
  Object.entries(p)
    ...
    .join("&");
const map = (r: UnknownRecord): ContentStrategyProject => ({
  id: r.id,
  workspaceId: r.workspace_id,
  name: r.name,
  keywordProjectId: r.keyword_project_id ?? undefined,
  competitorProjectId: r.competitor_project_id ?? undefined,
  status: r.status,
  summary: r.summary || {},
  strategy: r.strategy || {},
...
export async function createContentStrategy(
  ...
  const [r] = await supabaseRest<UnknownRecord[]>(ctx.req, "content_strategy_projects", {
    method: "POST",
    body: JSON.stringify({
      workspace_id: ctx.workspaceId,
      name: input.name,
      keyword_project_id: input.keywordProjectId || null,
      competitor_project_id: input.competitorProjectId || null,
      status: "ready",
...
export async function listContentStrategies(ctx: { req: NextRequest; workspaceId: string }) {
...
export async function getContentStrategy(ctx: { req: NextRequest; workspaceId: string }, id: string) {
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req,
    "content_strategy_projects",
    {},
