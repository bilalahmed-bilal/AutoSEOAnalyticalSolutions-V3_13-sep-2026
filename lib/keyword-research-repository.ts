...
export interface KeywordOpportunity extends KeywordCandidate {
  id: string;
  projectId: string;
  createdAt: string;
}
...
function mapProject(r: UnknownRecord): KeywordResearchProject {
  return {
    id: r.id,
...
function mapOpportunity(r: UnknownRecord): KeywordOpportunity {
  return {
    id: r.id,
    ...
    contentFitScore: r.content_fit_score,
    currentSignalScore: r.current_signal_score,
    tier: r.tier,
...
export async function createKeywordResearch(
  ...
  const [row] = await supabaseRest<UnknownRecord[]>(ctx.req, "keyword_research_projects", {
    method: "POST",
    body: JSON.stringify({
...
export async function listKeywordResearch(ctx: { req: NextRequest; workspaceId: string }) {
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req,
    "keyword_research_projects",
    {},
...
export async function getKeywordResearch(ctx: { req: NextRequest; workspaceId: string }, id: string) {
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req,
    "keyword_research_projects",
    {},
...
export async function replaceKeywordOpportunities(
  ctx: { req: NextRequest; workspaceId: string },
  projectId: string,
  ...
  await supabaseRest(
    ...
    { method: "DELETE" },
    q({ workspace_id: `eq.${ctx.workspaceId}`, project_id: `eq.${projectId}` })
  );
  if (!candidates.length) return [];
  const rows = await supabaseRest<UnknownRecord[]>(ctx.req, "keyword_opportunities", {
    method: "POST",
    body: JSON.stringify(
      candidates.map((c) => ({
        workspace_id: ctx.workspaceId,
        ...
        keyword: c.keyword,
        normalized_keyword: c.normalizedKeyword,
        source: c.source,
        intent: c.intent,
        relevance_score: c.relevanceScore,
        opportunity_score: c.opportunityScore,
        difficulty_score: c.difficultyScore,
        content_fit_score: c.contentFitScore,
        current_signal_score: c.currentSignalScore,
        tier: c.tier,
...
export async function listKeywordOpportunities(ctx: { req: NextRequest; workspaceId: string }, projectId: string) {
  const rows = await supabaseRest<UnknownRecord[]>(
    ctx.req,
    "keyword_opportunities",
    {},
    q({ workspace_id: `eq.${ctx.workspaceId}`, project_id: `eq.${projectId}`, order: "opportunity_score.desc" })
  );
  return rows.map(mapOpportunity);
}
