...
const corpus = (p: CrawlResult) =>
  [p.title || "", p.metaDescription || "", ...p.h1s, ...p.headingTexts, p.readableText, ...p.linkAnchors].join(" ");
...
function anchorFor(source: CrawlResult, target: CrawlResult, keywords: KeywordOpportunity[]) {
  ...
  const kw = keywords.find((k) => {
    const t = tokens(k.keyword);
    let h = 0;
    t.forEach((x) => {
      if (targetText.includes(x)) h++;
    });
    return t.size && h / t.size >= 0.6;
  });
  if (kw) return kw.keyword;
  return target.h1s[0] || target.title || path(target.url).split("/").filter(Boolean).pop() || "Learn more";
}
function isIndexable(p: CrawlResult) {
  return !p.noindex && p.statusCode >= 200 && p.statusCode < 400;
}

export async function analyzeSiteArchitecture(
  targetUrl: string,
  keywordOpportunities: KeywordOpportunity[] = [],
  maxPages = 30
): Promise<SiteArchitectureAnalysis> {
    ...
    .filter(
      (p) =>
        ...
        (p.internalLinkCount === 0 ||
          ![...pages].some((x) => x.url !== p.url && similarity(corpus(x), corpus(p)) > 0.72))
    )
    .map((p) => p.url);
  ...
  const hubs = pages
    .map((p) => {
      const score = Math.round(
        Math.min(100, p.internalLinkCount * 8 + Math.min(40, p.headingTexts.length * 4) + (p.wordCount > 800 ? 20 : 0))
      );
      return { url: p.url, score, incoming: incoming.get(p.url) || 0, outgoing: p.internalLinkCount };
    })
  ...
  for (const source of pages) {
    for (const target of pages) {
      ...
      const score = Math.min(
        100,
        Math.round(35 + sim * 55 + (targetOrphan ? 15 : 0) + (target.wordCount < 300 ? 5 : 0))
      );
      if (score < 55) continue;
      opportunities.push({
        type: targetOrphan ? "orphan" : "contextual",
        sourceUrl: source.url,
        targetUrl: target.url,
        anchorSuggestion: anchorFor(source, target, keywordOpportunities),
        score,
        severity: score >= 80 ? "high" : score >= 65 ? "medium" : "low",
        reason: targetOrphan
          ? "The target page appears weakly connected to the crawled site; add a contextual link from a closely related page."
          : "The source and target have meaningful topical overlap; a contextual internal link may improve discovery and topic relationships.",
      });
    }
  }
  const dedup = new Map<string, LinkOpportunity>();
  for (const o of opportunities) {
    const k = `${o.sourceUrl}|${o.targetUrl}`;
  ...
  const architectureScore = Math.max(
    ...
    Math.min(
      ...
      Math.round(
        100 -
          (orphanPages.length / Math.max(1, pages.length)) * 35 -
          (weakPages.length / Math.max(1, pages.length)) * 20 -
          (pages.length > 0 && internalLinks / pages.length < 2 ? 20 : 0)
      )
    )
  );
  const recommendations = [
    ...(orphanPages.length
      ? [
          `Prioritize ${orphanPages.length} weakly connected page(s) and add relevant contextual links from established pages.`,
        ]
      : []),
    ...(weakPages.length
      ? [`Review ${weakPages.length} page(s) with very few internal links or thin crawlable content.`]
      : []),
    ...(hubs.length
      ? [`Use strong hub pages such as “${hubs[0].url}” to distribute links to closely related supporting pages.`]
      : []),
  ...
  return {
    ...
    summary: {
      ...
      linkOpportunities: final.length,
