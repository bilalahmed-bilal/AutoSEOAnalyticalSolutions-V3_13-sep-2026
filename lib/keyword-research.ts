...
const STOP_WORDS = new Set("a an and are as at be by for from how i in is it of on or that the this to was what when where which who why with your you our we official online guide best".split(/\s+/));
const QUESTION_STARTS = ["what is", "how to", "how do", "why", "when", "where", "which", "can i", "is", "are"];
const MODIFIERS = ["best", "cheap", "price", "pricing", "online", "near me", "services", "guide", "tips", "reviews", "comparison", "vs", "alternative", "for beginners"];
...
function words(value: string) { return normalize(value).split(" ").filter(Boolean); }
function unique(values: string[]) { return [...new Set(values.map(normalize).filter(Boolean))]; }
...
export function detectKeywordIntent(keyword: string): KeywordIntent {
  ...
  const transactional = /\b(buy|order|purchase|book|hire|price|pricing|cheap|deal|quote|subscribe|service|services)\b/.test(k);
  const commercial = /\b(best|top|review|reviews|compare|comparison|vs|alternative|alternatives)\b/.test(k);
  const informational = /\b(what|why|how|guide|tutorial|learn|meaning|definition|tips|examples|when|where|which)\b/.test(k);
...
function contentFit(keyword: string, crawl?: CrawlResult | null) {
  ...
  const pageText = normalize([crawl.title, crawl.metaDescription, ...crawl.h1s, ...(crawl.headingTexts || []), ...(crawl.readableText || "").slice(0, 12000)].join(" "));
...
function buildCandidate(keyword: string, source: KeywordCandidate["source"], seed: string, crawl?: CrawlResult | null): KeywordCandidate {
  const normalizedKeyword = normalize(keyword);
  const intent = detectKeywordIntent(normalizedKeyword);
  const relevanceBase = normalize(seed).split(" ").filter(Boolean).reduce((n, token) => normalizedKeyword.includes(token) ? n + 1 : n, 0);
  const relevanceScore = clamp(45 + (relevanceBase / Math.max(1, words(seed).length)) * 45 + (source === "gsc" ? 10 : 0));
  ...
  const opportunityScore = clamp(relevanceScore * 0.30 + intentScore * 0.20 + contentFitScore * 0.20 + (100 - difficultyScore) * 0.20 + (100 - currentSignalScore) * 0.10);
  const tier: OpportunityTier = opportunityScore >= 80 ? "priority" : opportunityScore >= 68 ? "strong" : opportunityScore >= 52 ? "watch" : "low";
  const type = contentType(intent, normalizedKeyword);
  const recommendation = currentSignalScore >= 70
  ...
  return { keyword, normalizedKeyword, source, intent, relevanceScore, opportunityScore, difficultyScore, contentFitScore, currentSignalScore, tier, recommendedContentType: type, recommendation };
}

export function generateKeywordCandidates(seedKeyword: string, crawl?: CrawlResult | null, gscQueries: string[] = []): KeywordCandidate[] {
  ...
  if (crawl) {
    ...
    for (const anchor of crawl.linkAnchors || []) {
      ...
      if (normalized && normalized.split(" ").length <= 10 && seedWords.some((w) => normalized.includes(w))) raw.push([normalized, "page"]);
    }
  }
  ...
  return [...dedup.values()].sort((a, b) => b.opportunityScore - a.opportunityScore || a.difficultyScore - b.difficultyScore).slice(0, 100);
}

export function summarizeKeywordResearch(candidates: KeywordCandidate[]) {
  const counts = candidates.reduce((acc, c) => { acc[c.tier] += 1; return acc; }, { priority: 0, strong: 0, watch: 0, low: 0 } as Record<OpportunityTier, number>);
  const intents = candidates.reduce((acc, c) => { acc[c.intent] = (acc[c.intent] || 0) + 1; return acc; }, {} as Record<string, number>);
  return {
    ...
    low: counts.low,
    intentMix: intents,
    topOpportunities: candidates.slice(0, 10).map((c) => ({ keyword: c.keyword, score: c.opportunityScore, intent: c.intent, type: c.recommendedContentType }))
  };
}
