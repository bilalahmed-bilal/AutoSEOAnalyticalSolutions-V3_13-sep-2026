...
export function analyzeKeyword(crawl: CrawlResult, keyword: string): KeywordIntelligenceResult {
  ...
  const titlePosition: KeywordIntelligenceResult["metrics"]["title"]["position"] =
    titleIndex < 0 ? "none" : titleIndex <= Math.max(0, Math.floor(title.length / 3)) ? "start" : titleIndex >= Math.floor(title.length * 0.66) ? "end" : "middle";
  const titleExact = titleIndex >= 0;
  const h1Exact = crawl.h1s.some((h) => hasMatch(h, normalizedKeyword));
  const headingTexts = [...crawl.h1s];
  ...
  const exactUrl = normalize(new URL(crawl.url).pathname.replace(/[-_]+/g, " ")).includes(normalizedKeyword.replace(/[-_]+/g, " "));
  ...
  deduct(!titleExact, 20, { severity: "high", issue: "Target keyword is not present in the page title.", fix: "Include the target keyword naturally in the title, preferably near the beginning when it reads well.", points: 20 });
  deduct(!h1Exact, 15, { severity: "high", issue: "Target keyword is not present in an H1.", fix: "Align the primary H1 with the page topic and target keyword without forcing an exact-match phrase.", points: 15 });
  deduct(!hasMatch(crawl.metaDescription, normalizedKeyword), 10, { severity: "medium", issue: "Target keyword is absent from the meta description.", fix: "Mention the keyword naturally while keeping the description useful and click-oriented.", points: 10 });
  deduct(crawl.h2Count === 0, 6, { severity: "low", issue: "No H2 headings were detected for semantic topic organization.", fix: "Use descriptive H2 sections that cover important subtopics and related concepts.", points: 6 });
  deduct(!exactUrl, 5, { severity: "low", issue: "The target keyword is not reflected in the URL path.", fix: "Consider a short, readable keyword-aligned slug when changing the URL is safe and justified.", points: 5 });
  deduct(crawl.wordCount < 300, 8, { severity: "medium", issue: "The page has limited crawlable text for meaningful topic coverage.", fix: "Expand the page with genuinely useful sections that satisfy the search intent.", points: 8 });
  deduct(crawl.internalLinkCount < 2, 4, { severity: "low", issue: "Few internal links support this page's topic authority.", fix: "Add relevant internal links from and to closely related pages.", points: 4 });

  const recommendations = [
    `Search intent signal: ${detectIntent(normalizedKeyword)}. Make the page's main purpose obvious above the fold.`,
    titleExact ? "Keep the keyword in the title only if it remains natural and compelling." : "Add the target keyword to the title without keyword stuffing.",
    h1Exact ? "Keep the H1 focused on the single primary topic." : "Create one clear H1 aligned with the target topic.",
    crawl.h2Count > 0 ? "Use H2s to answer distinct subtopics and related questions." : "Add H2 sections for important subtopics and questions.",
  ...
  return {
    ...
    metrics: {
      ...
      metaDescription: { present: Boolean(crawl.metaDescription), exactMatch: hasMatch(crawl.metaDescription, normalizedKeyword) },
      h1: { present: crawl.h1s.length > 0, exactMatch: h1Exact },
      headings: { matches: headingMatches, total: crawl.h1s.length + crawl.h2Count },
      body: { wordCount: crawl.wordCount, exactOccurrences: contentSignal, densityPercent: Number(densityPercent.toFixed(2)), prominent: contentSignal >= 2 },
      url: { present: Boolean(new URL(crawl.url).pathname && new URL(crawl.url).pathname !== "/"), exactMatch: exactUrl },
