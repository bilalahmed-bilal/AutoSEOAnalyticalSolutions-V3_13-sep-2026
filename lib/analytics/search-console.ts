...
async function listSites(accessToken: string): Promise<SearchConsoleSite[]> {
  ...
  return (data.siteEntry || [])
    .map((s: UnknownRecord) => ({ siteUrl: String(s.siteUrl || ""), permissionLevel: String(s.permissionLevel || "") }))
    .filter((s: SearchConsoleSite) => s.siteUrl);
}
...
export async function getSearchConsoleSummary(settings: SearchConsoleSettings, startDate: string, endDate: string) {
  ...
  return {
    ...
    periodEnd: endDate,
  } satisfies SearchConsoleSummary;
}
...
export async function getSearchConsoleConnection(workspaceId: string) {
  const rows = await supabaseAdmin<UnknownRecord[]>(
    "connections",
    {},
    `?workspace_id=eq.${encodeURIComponent(workspaceId)}&provider=eq.google-search-console&status=neq.revoked&limit=1`
  );
  const row = rows[0];
  if (!row) return null;
