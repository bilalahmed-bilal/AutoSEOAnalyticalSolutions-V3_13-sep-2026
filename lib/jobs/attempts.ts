...
export async function startJobAttempt(jobId: string, workspaceId: string, attempt: number, workerId: string) {
  const [row] = await supabaseAdmin<any[]>("job_attempts", {
    method: "POST",
    body: JSON.stringify({ job_id: jobId, workspace_id: workspaceId, attempt, worker_id: workerId, status: "running" }),
...
export async function finishJobAttempt(input: {
  jobId: string; attempt: number; status: "succeeded" | "failed"; errorMessage?: string; providerJobId?: string; providerLink?: string;
}) {
  const rows = await supabaseAdmin<any[]>("job_attempts", {
    method: "PATCH",
    body: JSON.stringify({
      status: input.status,
