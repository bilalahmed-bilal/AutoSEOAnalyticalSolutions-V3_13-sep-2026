...
| G-07 | Broken doc links | README YouTube/Facebook setup now points at existing `docs/INTEGRATIONS.md`. Other historical V3–V19 paths may still be missing. | Broken onboarding | P1 | Point remaining README paths at existing consolidated docs. | `README.md`, `docs/*` | Every README path exists | PARTIAL |
...
| G-30 | YouTube Analytics scope | OAuth config requests `yt-analytics.readonly`. Refresh maps DB provider `youtube` to Google YouTube config. Old tokens without the scope must reconnect; APIs throw instead of inventing data. Live Analytics API call **NOT VERIFIED**. | Honest reconnect path | P3 | Keep reconnect error; live Google grant still environment-specific. | `lib/oauth/config.ts`, `lib/oauth/lifecycle.ts` | Unit/source tests. Live Analytics **NOT VERIFIED** | PARTIAL |
