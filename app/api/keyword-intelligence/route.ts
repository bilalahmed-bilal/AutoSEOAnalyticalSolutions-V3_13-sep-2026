...
export async function POST(req: NextRequest) {
  ...
  if (!rate.ok) return NextResponse.json({ error: "Too many keyword analysis requests." }, { status: 429, headers: { "Retry-After": String(rate.retryAfterSeconds) } });
  try {
    ...
    writeAudit({ action: "seo_analysis", actor: access.user?.id || "anonymous-local", metadata: { url: crawl.url, keyword: body.keyword.trim(), score: intelligence.score } });
    return NextResponse.json({ crawl, intelligence });
  } catch (err: any) {
    console.error("keyword intelligence error:", err);
    return NextResponse.json({ error: err?.message || "Keyword analysis failed." }, { status: 400 });
  }
}
