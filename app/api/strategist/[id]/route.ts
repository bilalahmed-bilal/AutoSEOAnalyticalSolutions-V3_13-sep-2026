...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!t) return NextResponse.json({ error: "Valid workspace required." }, { status: 400 });
  const { id } = await params;
  const run = await getStrategyRun(t.workspaceId, id);
  if (!run) return NextResponse.json({ error: "Strategy run not found." }, { status: 404 });
