...
export async function POST(req: NextRequest) {
  ...
  if (!connection)
    return NextResponse.json({ error: "Google Search Console connection not configured." }, { status: 404 });
  try {
    ...
    return NextResponse.json({ error: errorMessage(e, "Search Console snapshot failed.") }, { status: 502 });
  }
}
