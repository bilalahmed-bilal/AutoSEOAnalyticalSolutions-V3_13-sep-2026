...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (e.status !== "running_a")
    return NextResponse.json({ error: "Variant A must be running before Variant B starts." }, { status: 409 });
  const b = await req.json().catch(() => ({}));
  const end = String(b.aEndDate || "");
  if (!/^\d{4}-\d{2}-\d{2}$/.test(end))
    return NextResponse.json({ error: "aEndDate (YYYY-MM-DD) is required." }, { status: 400 });
  try {
    await updateExperiment(p.tenant.workspaceId, id, { variant_a_end: end, variant_b_start: nextDay(end) });
    return NextResponse.json(await queueExperimentVariant({ req, workspaceId: p.tenant.workspaceId }, id, "b"));
  } catch (err: unknown) {
    return NextResponse.json({ error: errorMessage(err, "Variant B start failed.") }, { status: 400 });
  }
}
