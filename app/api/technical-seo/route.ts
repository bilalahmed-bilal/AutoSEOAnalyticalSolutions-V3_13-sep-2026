...
export async function GET(req: NextRequest) {
  ...
  try {
    return Response.json({ audits: await listTechnicalAudits({ req, workspaceId: tenant.workspaceId }) });
  } catch (e: unknown) {
    return Response.json({ error: errorMessage(e, "Unable to load audits.") }, { status: 500 });
  }
}
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const saved = await saveTechnicalAudit(
      { req, workspaceId: tenant.workspaceId },
      {
        targetUrl: result.site.startUrl,
        score: result.score,
        summary,
        report: { site: result.site, fixes: result.fixes },
    ...
    return Response.json({ error: errorMessage(e, "Technical SEO audit failed.") }, { status: 400 });
  }
}
