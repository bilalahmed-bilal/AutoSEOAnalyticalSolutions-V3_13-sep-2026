...
export async function GET(req: NextRequest) {
  ...
  const rows = await supabaseRest<any[]>(req, "audit_logs", {}, `?workspace_id=eq.${tenant.workspaceId}&select=id,actor_user_id,action,entity_type,entity_id,metadata,created_at&order=created_at.desc&limit=100`);
