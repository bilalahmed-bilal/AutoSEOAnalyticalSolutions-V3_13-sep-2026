...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (version.status !== "proposed") return NextResponse.json({ error: "Only proposed versions can be approved." }, { status: 409 });
  const approved = applyDecisions(version.original, version.optimized, version.decisions);
  const draft = await updateDraftRemote({ req, workspaceId: tenant.workspaceId }, version.draftId, { title: approved.title, body: approved.body, metaDescription: approved.metaDescription, status: "approved" });
  if (!draft) return NextResponse.json({ error: "Draft not found." }, { status: 404 });
  await updateDraftVersionRemote({ req, workspaceId: tenant.workspaceId }, id, { status: "approved", approved_at: new Date().toISOString() });
  const job = await enqueueJob({ workspaceId: tenant.workspaceId, type: "publish_draft", payload: { draftId: version.draftId, sourceVersionId: id }, idempotencyKey: publishJobKey(tenant.workspaceId, version.draftId) });
  const applied = await updateDraftVersionRemote({ req, workspaceId: tenant.workspaceId }, id, { status: "applied", applied_at: new Date().toISOString() });
