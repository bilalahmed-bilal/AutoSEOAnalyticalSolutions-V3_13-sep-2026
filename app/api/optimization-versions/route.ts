...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    versions: await listDraftVersionsRemote({ req, workspaceId: permission.tenant.workspaceId }, draftId),
  });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = (await req.json()) as {
      ...
      optimized?: Partial<Record<string, string>>;
      changes?: string[];
      warnings?: string[];
    };
    if (!body.draftId || !body.original || !body.optimized)
      return NextResponse.json({ error: "draftId, original and optimized are required." }, { status: 400 });
    const original = normalizeSnapshot(body.original);
    const optimized = normalizeSnapshot(body.optimized);
    const decisions = buildInitialDecisions(original, optimized);
    const version = await createDraftVersionRemote(
    ...
    return NextResponse.json({ error: errorMessage(err, "Version create failed.") }, { status: 400 });
  }
}
