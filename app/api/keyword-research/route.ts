...
import {
  ...
  listKeywordResearch,
  replaceKeywordOpportunities,
} from "@/lib/keyword-research-repository";
...
export async function POST(req: NextRequest) {
  ...
  const seed = String(body.seedKeyword || "").trim();
  const targetUrl = String(body.targetUrl || "").trim();
  if (!seed) return NextResponse.json({ error: "seedKeyword is required." }, { status: 400 });
  let crawl: UnknownRecord = null;
  if (targetUrl) {
    try {
      crawl = (await crawlSite(targetUrl, 1)).pages?.[0] || null;
    } catch (e: unknown) {
      return NextResponse.json(
        { error: `Target URL crawl failed: ${errorMessage(e, "Unable to crawl URL.")}` },
        { status: 400 }
      );
    }
  }
  ...
  try {
    ...
    const r = await getSearchConsoleKeywordQueries(
      p.tenant.workspaceId,
      start.toISOString().slice(0, 10),
      end.toISOString().slice(0, 10),
      seed
    );
    gscQueries = r.queries.map(String);
    gscConnected = r.connected;
  } catch {
    /* GSC is optional; local discovery remains usable. */
  }
  const candidates = generateKeywordCandidates(seed, crawl, gscQueries);
  const summary = {
    ...summarizeKeywordResearch(candidates),
    gscConnected,
    gscQueryCount: gscQueries.length,
    targetUrl: targetUrl || null,
  };
  const project = await createKeywordResearch(
    { req, workspaceId: p.tenant.workspaceId },
    {
      name: String(body.name || `${seed} keyword research`).trim(),
      seedKeyword: seed,
      targetUrl,
      status: "ready",
      summary,
  ...
  const opportunities = await replaceKeywordOpportunities(
    { req, workspaceId: p.tenant.workspaceId },
    project.id,
    candidates
  );
