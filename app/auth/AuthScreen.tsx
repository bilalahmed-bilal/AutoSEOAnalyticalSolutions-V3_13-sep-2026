    ...
    <section className="mx-auto max-w-md border border-line bg-white/80 p-6 shadow-sm">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4">
        ...
        {mode !== "forgot" && (
          <label className="block text-sm">
            Password
            <div className="mt-1 flex border border-line bg-white">
              <input
                required
                minLength={6}
              ...
              />
              <button type="button" onClick={() => setShow(!show)} className="px-3 text-xs text-ink/60">
                {show ? "Hide" : "Show"}
              </button>
            </div>
          </label>
        ...
        {message && <p className="border border-green-300 bg-green-50 p-2 text-sm text-green-700">{message}</p>}
        <button
          disabled={busy}
